...
class ProblemDefinition(object):
    ...
    def is_state_high_quality(self, J):
        ...
        """
        ########## TODO ##########
        # only criterion is volume-based manipulability > 0.01
        # V = (4/3)pi * sqrt(lambda1*lambda2*...lambdaM) where each lambda is an eigenvalue of JJ^T
        # V = (4/3)Pi * Det(JJT)
        #print("I'm the problem")
        A = np.matmul(J, np.transpose(J))
        if ((4/3)*np.pi*np.sqrt(np.linalg.det(A))) > 0.01:
            return True
        return False
